...
<div class="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-4">
    <div>
        <h2 class="text-4xl font-black text-gray-900 tracking-tighter mb-2 uppercase italic">Statistik <span class="text-blue-600">Inventaris</span></h2>
        <p class="text-gray-500 font-medium italic">Data real-time stok fisik.</p>
    </div>
...
<div class="bg-white rounded-[3.5rem] shadow-xl border border-gray-100 overflow-hidden">
    <div class="p-10 border-b border-gray-50 flex justify-between items-center bg-gray-50/50">
        <h3 class="font-black text-gray-800 flex items-center gap-4 text-xl tracking-tighter uppercase italic">Prioritas Re-Stock</h3>
        <a href="<?= base_url('warehouse/inventory') ?>" class="bg-gray-900 text-white px-8 py-4 rounded-[1.5rem] text-xs font-black hover:bg-blue-600 transition-all shadow-xl active:scale-95 uppercase tracking-widest">
            Buka Inventori
        </a>
    </div>
                ...
                if (lowStockItems.length > 0) {
                    tbody.innerHTML = lowStockItems.map(item => `
                        <tr class="bg-white hover:bg-blue-50/30 transition-all duration-300 shadow-sm border border-gray-50">
                            <td class="px-10 py-6 rounded-l-[2.5rem] border-y border-l border-gray-50">
                                <div class="flex items-center gap-6">
                                    <div>
                            ...
                            <td class="px-10 py-6 text-right rounded-r-[2.5rem] border-y border-r border-gray-100">
                                <a href="<?= base_url('warehouse/inventory') ?>?edit=${item.id}" class="bg-gray-900 text-white px-6 py-3 rounded-2xl text-[10px] font-black hover:bg-blue-600 transition-all uppercase italic">Update Stok</a>
                            </td>
                        </tr>
                    `).join('');
                } else {
